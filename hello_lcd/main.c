...
int main(void) {
    ...
    uint8_t gpio_DIN = 3; // GP3 SPI0 TX
    uint8_t gpio_DC  = 4; // GP4 not using SPI0 RX
    uint8_t gpio_CS  = 5; // GP5 not using SPI0 CSn
    uint8_t gpio_RST = 6; // GP6
    ...
    master_spi_t* m_spi = master_spi_create(spi0, 1, gpio_DIN, gpio_CLK);
